...
const SignUpPassword = ({ onSubmit, onBack }) => {
  ...
  const handleIdChange = async e => {
    ...
    try {
      const res = await api.post('/app-user/initial/redundancy-check', { loginId: val });
      if (res.data === true) {
  ...
  return (
    <div className={scss.wrapper}>
      ...
      <div className={scss.container}>
        ...
        <InputField label="아이디" value={id} onChange={handleIdChange} isValid={isIdValid} errorMessage={idError} successMessage="사용 가능한 아이디입니다" Icon={isIdValid ? CheckIcon : DeniedIcon} />
        <InputField label="비밀번호" value={password} type="password" onChange={handlePasswordChange} isValid={isPwValid} errorMessage={pwError} successMessage="사용 가능한 비밀번호입니다" Icon={isPwValid ? CheckIcon : DeniedIcon} />
        <InputField label="비밀번호 확인" value={confirm} type="password" onChange={handleConfirmChange} isValid={isConfirmValid} errorMessage={confirmError} successMessage="비밀번호가 일치합니다" Icon={isConfirmValid ? CheckIcon : DeniedIcon} />
      </div>
